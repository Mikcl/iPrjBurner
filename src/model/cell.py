...
def get_perception_vector(x, channels , angle=0.0):
  ...
  i_kernel = identify[None, None, ...].repeat(channels, 1, 1, 1).to(device)  # TODO this will always be the same.
  i_v = nn.functional.conv2d(x, i_kernel, padding=1, groups=channels)

  x_kernel = x_direction[None, None, ...].repeat(channels, 1, 1, 1).to(device)
  x_v = nn.functional.conv2d(x, x_kernel, padding=1, groups=channels)
  y_kernel = y_direction[None, None, ...].repeat(channels, 1, 1, 1).to(device)
  y_v = nn.functional.conv2d(x, y_kernel, padding=1, groups=channels)
...
class Cell(nn.Module):
    ...
    def perceive(self, x, angle=0.0):
      stacked_image = get_perception_vector(x, self.channel_n, angle)
      return stacked_image
